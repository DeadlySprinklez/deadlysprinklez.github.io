function tabSwitch(e) {
	...
	if (doc.getElementsByClassName(target)[0].className.includes("active")) {
		return null;
	}

	let test = doc.getElementsByClassName("active")[0];
	let prevTab = 0;
	for (i of test.classList) {
		if (i.length == 2) {
			prevTab = i;
			break;
		}
...
function changeToExpression(e) {
	// okay so when changing expressions we should check for any discrepancies
	// between the expressionArray length and the amount of elements in the list
	let target = e.target;
...
onmousedown = (e) => {
	...
	if (e.target == sheetDivider || e.target == rowPrevPreview || e.target == dialoguePreview) {
		if (imgUploaded) {
			isMouseDown = true;
